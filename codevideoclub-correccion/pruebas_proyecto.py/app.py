...
class RegistrationForm(FlaskForm):
    ...
    role = SelectField('Role', choices=[('manager', 'Manager'), ('cliente', 'Cliente')], validators=[DataRequired()], default="cliente")
...
@app.route('/editar_usuarios', methods=['GET', 'POST'])
def editar_usuarios():
    form = RegistrationForm()
    if form.validate_on_submit():
        user = User(username=form.username.data, role=form.role.data)
        ...
        db.session.commit()
        return redirect(url_for('index'))
    return redirect(url_for('editar_usuarios', form=form))